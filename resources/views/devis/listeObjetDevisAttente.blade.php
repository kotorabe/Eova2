...
@section('content')
                            ...
                            <tbody class="text-center">
                                ...
                                <tr>
                                    ...
                                    <td scope="row"><a href="#" id="mapLink" data-bs-toggle="modal"
                                            ...
                                            data-livr="{{ $utilisateur->coord_livr }}" onclick="openMap()">
                                    ...
                                    <td scope="row"><a href="#" id="mapLinkLivr" data-bs-toggle="modal"
                                            ...
                                            data-recup="{{ $utilisateur->coord_recup }}"
                                            data-livr="{{ $utilisateur->coord_livr }}" onclick="openMapLivr()">
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            const prixInputs = document.querySelectorAll('.form-control[name="prix"]');
            prixInputs.forEach(function(input) {
                const row = input.closest('tr');
                const addButton = row.querySelector('.btn-success');
                const saveButton = row.querySelector('.btn-warning');

                // Vérifier la valeur initiale du champ de prix
                const prixInitial = parseFloat(input.value.trim());
                if (isNaN(prixInitial) || prixInitial === 0 ) { // Désactive les boutons si le prix initial n'est pas un nombre ou s'il est égal à zéro
                    addButton.disabled = true;
                    // saveButton.disabled = true;
                }
                if (isNaN(prixInitial) || (prixInitial != 0 || prixInitial === '')) {
                    saveButton.disabled = true;
                }

                input.addEventListener('input', function() {
                    const prix = parseFloat(this.value.trim());
                    if(prixInitial === 0){ //ajouter
                        if (isNaN(prix) ){ // Désactive les boutons si le prix n'est pas un nombre ou s'il est égal à zéro
                            addButton.disabled = true;
                        }
                    }

                    if ((prix != prixInitial) && (prixInitial != 0 || prixInitial === '')) {
                        saveButton.disabled = false;
                    }
                    if (prix === prixInitial && prixInitial != 0) {
                        saveButton.disabled = true;
                    } else {
